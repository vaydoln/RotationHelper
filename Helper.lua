...
-----------------------------------------------------------------
--- Spell mappings required to get some spells to show up 
--- correctly regarding cooldowns and such
-----------------------------------------------------------------
...
function RotationHelper:IntUnitAura(unit, nameOrId, filter, timeShift)
	local aura = {
		name           = nil,
		up             = false,
		upMath         = 0,
...
function RotationHelper:CollectAura(unit, timeShift, output, filter)
...
RotationHelper.PlayerAuras = setmetatable({}, auraMetaTable);
RotationHelper.TargetAuras = setmetatable({}, auraMetaTable);

function RotationHelper:CollectAuras(timeShift)
	self:CollectAura('player', timeShift, self.PlayerAuras);
	self:CollectAura('target', timeShift, self.TargetAuras);
	return self.PlayerAuras, self.TargetAuras;
end

function RotationHelper:DumpAuras()
	print('Player Auras');
...
function RotationHelper:SpecName()
	local currentSpec = GetSpecialization();
	local currentSpecName = currentSpec and select(2, GetSpecializationInfo(currentSpec)) or 'None';
	return currentSpecName;
end

function RotationHelper:CheckTalents()
	self.PlayerTalents = {};
...
RotationHelper.isMelee = false;
function RotationHelper:CheckIsPlayerMelee()
	self.isMelee = false;
...
function RotationHelper:HasTalent(talent)
	return self.PlayerTalents[talent];
end

function RotationHelper:TalentEnabled(talent)
	self:Print(self.Colors.Error .. 'RotationHelper:TalentEnabled is deprecated, please use table `talents` to check talents');
end

function RotationHelper:GetAzeriteTraits()
...
function RotationHelper:GetAzeriteEssences()
...
---
function RotationHelper:GetCovenantInfo()
...
-- TODO: finish this
function RotationHelper:GetLegendaryEffects()
...
function RotationHelper:DumpAzeriteTraits()
...
-- @deprecated
function RotationHelper:UnitAura(auraId, timeShift, unit, filter)
...
-- Aura on player
function RotationHelper:Aura(name, timeShift)
	return self:UnitAura(name, timeShift, 'player');
end


-- Aura on target
function RotationHelper:TargetAura(name, timeShift)
...
function RotationHelper:EndCast(target)
...
function RotationHelper:GlobalCooldown(spellId)
	local baseGCD = 1.5;
	if spellId then
...
function RotationHelper:AttackHaste()
	local haste = UnitSpellHaste('player');
...
function RotationHelper:ItemCooldown(itemId, timeShift)
...
function RotationHelper:CooldownConsolidated(spellId, timeShift)
	timeShift = timeShift or 0;
	local remains = 100000;
	local t = GetTime();
   local spellName = spellIdMapping[spellId] or spellId;
   ...
   if charges == nil then
	...
	return {
		duration        = GetSpellBaseCooldown(spellName) / 1000,
		ready           = remains <= 0,
		remains         = remains,
...
function RotationHelper:TargetPercentHealth(unit)
...
function RotationHelper:SetBonus(items)
	local c = 0;
...
function RotationHelper:Mana(minus, timeShift)
	local _, casting = GetManaRegen();
	local mana = UnitPower('player', 0) - minus + (casting * timeShift);
	return mana / UnitPowerMax('player', 0), mana;
end

function RotationHelper:ExtractTooltip(spell, pattern)
	local _pattern = gsub(pattern, "%%s", "([%%d%.,]+)");

	if not RotationHelperSpellTooltip then
		CreateFrame('GameTooltip', 'RotationHelperSpellTooltip', UIParent, 'GameTooltipTemplate');
		RotationHelperSpellTooltip:SetOwner(UIParent, "ANCHOR_NONE")
	end
	RotationHelperSpellTooltip:SetSpellByID(spell);

	for i = 2, 4 do
		local line = _G['RotationHelperSpellTooltipTextLeft' .. i];
...
function RotationHelper:Bloodlust(timeShift)
	-- @TODO: detect exhausted/seated debuff instead of 6 auras
	for k, v in pairs(_Bloodlusts) do
		if RotationHelper:Aura(v, timeShift or 0) then
			return true;
		end
	end
...
RotationHelper.Spellbook = {};
function RotationHelper:FindSpellInSpellbook(spell)
	local spellName = GetSpellInfo(spell);
	if RotationHelper.Spellbook[spellName] then
		return RotationHelper.Spellbook[spellName];
	end
	...
	for index = offset + 1, numSpells + offset do
		...
		if spellID and spellName == GetSpellBookItemName(index, booktype) then
			RotationHelper.Spellbook[spellName] = index;
			return index;
		end
	end
...
function RotationHelper:IsSpellInRange(spell, unit)
	unit = unit or 'target';
	...
	if inRange == nil then
		local bookType = 'spell';
		local myIndex = RotationHelper:FindSpellInSpellbook(spell)
		if myIndex then
...
function RotationHelper:TargetsInRange(spell)
	local count = 0;

	for _, unit in ipairs(self.visibleNameplates) do
		if RotationHelper:IsSpellInRange(spell, unit) == 1 then
			count = count + 1;
		end
	end
...
function RotationHelper:ThreatCounter()
	local count = 0;
	local units = {};
...
function RotationHelper:DebuffCounter(spellId, timeShift)
	local count, totalRemains, totalCount, totalCountRemains = 0, 0, 0, 0;

	for _, unit in ipairs(self.visibleNameplates) do
		local aura = RotationHelper:IntUnitAura(unit, spellId, 'PLAYER|HARMFUL', timeShift);
		if aura.up then
			count = count + 1;
...
function RotationHelper:SmartAoe(itemId)
...
function RotationHelper:FormatTime(left)
