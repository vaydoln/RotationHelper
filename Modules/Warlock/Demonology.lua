...
function Warlock:createDemonologyEffectsTable()
   local effects = {};
   ...
   --    local expected = 23;
   --    if fd.talents[HV.InsatiableHunger] then
   --       expected = 28;
   --    end
...
local DEEffect = Warlock:createDemonologyEffectsTable();

function Warlock:DemonologyPrep(fd)
	fd.soulShards = UnitPower('player', Enum.PowerType.SoulShards, true) / 10;
   fd.soulShardsMax = 5;
   return fd;
end

function Warlock:Demonology(fd)
   local spellId = Warlock:chooseDemonologySpellId(fd);
   local retVal = {
      id = spellId,
   };

   if (spellId and DEEffect[spellId]) then
      retVal.updateFrameData = DEEffect[spellId];
   end

   return retVal;
end

function Warlock:chooseDemonologySpellId(fd)
	...
	local petAura = RotationHelper:IntUnitAura('pet', DE.Felstorm, nil, timeShift);
	...
	if talents[DE.GrimoireFelguard] then
		RotationHelper:GlowCooldown(DE.GrimoireFelguard,
	...
	if cooldown[DE.SummonDemonicTyrant].remains < 4 and not tyrantReady then
		return Warlock:DemonologyTyrantPrep(fd);
	end

	-- run_action_list,name=summon_tyrant,if=variable.tyrant_ready;
	if tyrantReady then
		return Warlock:DemonologySummonTyrant(fd);
	end
	...
	then
		local result = Warlock:DemonologyCovenant(fd);
		if result then
			return result;
		end
	end
...
function Warlock:DemonologyCovenant(fd)
	local cooldown = fd.cooldown;
	local currentSpell = fd.currentSpell;
	local talents = fd.talents;
...
function Warlock:DemonologySummonTyrant(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local currentSpell = fd.currentSpell;
	local talents = fd.talents;
...
function Warlock:DemonologyTyrantPrep(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local currentSpell = fd.currentSpell;
	local talents = fd.talents;
