...
function Warlock:createAfflictionEffectsTable()
   ...
   effects[AF.SeedOfCorruption] = function(fd)
      fd.soulShards = fd.soulShards - 1;
      return fd;
   end

   effects[AF.MaleficRapture] = function(fd)
      fd.soulShards = fd.soulShards - 1;
      return fd;
   end
   ...
   effects[AF.Haunt] = function(fd)
      fd = RotationHelper:addTargetDebuff(fd, AF.Haunt);
      fd = RotationHelper:startCooldown(fd, AF.Haunt);
      return fd;
   end

   effects[AF.VileTaint] = function(fd)
      fd.soulShards = fd.soulShards - 1;
      fd = RotationHelper:addTargetDebuff(fd, AF.VileTaint);
      return fd;
   end

   effects[AF.Agony] = RotationHelper:normalAddTargetDebuff(AF.Agony);
   effects[AF.Corruption] = RotationHelper:normalAddTargetDebuff(AF.CorruptionAura);
   effects[AF.SowTheSeeds] = RotationHelper:normalAddTargetDebuff(AF.CorruptionAura);
   effects[AF.SiphonLife] = RotationHelper:normalAddTargetDebuff(AF.SiphonLife);
   effects[AF.UnstableAffliction] = RotationHelper:normalAddTargetDebuff(AF.UnstableAffliction);

   effects[AF.SoulRot] = RotationHelper:normalCooldownEffect(AF.SoulRot);
   effects[AF.PhantomSingularity] = RotationHelper:normalCooldownEffect(AF.PhantomSingularity);
   effects[AF.SummonDarkglare] = RotationHelper:normalCooldownEffect(AF.SummonDarkglare);
   effects[AF.DarkSoulMisery] = RotationHelper:normalCooldownEffect(AF.DarkSoulMisery);

   return effects;
end

local AFEffect = Warlock:createAfflictionEffectsTable();

function Warlock:AfflictionPrep(fd)
	fd.soulShards = UnitPower('player', Enum.PowerType.SoulShards, true) / 10;
   fd.soulShardsMax = 5;

   if (fd.currentSpell and AFEffect[fd.currentSpell]) then
      local updateFrameData = AFEffect[fd.currentSpell];
...
function Warlock:Affliction(fd)
   local spellId = Warlock:chooseAfflictionSpellId(fd);
   local retVal = {
      id = spellId,
   };
...
function Warlock:chooseAfflictionSpellId(fd)
	...
	if talents[AF.DarkSoulMisery] then
		RotationHelper:GlowCooldown(AF.DarkSoulMisery, cooldown[AF.DarkSoulMisery].ready);
	end

	-- call_action_list,name=aoe,if=active_enemies>3;
	if targets > 3 then
		local result = Warlock:AfflictionAoe(fd);
		if result then
			return result;
		end
	end
	...
	if not darkglareAsCooldown then
		...
		then
			local result = Warlock:AfflictionDarkglarePrep(fd);
			if result then
				return result;
			end
		end
   ...
   if debuff[AF.Agony].remains < 4 then
	...
	if covenantId ~= Necrolord then
		local result = Warlock:AfflictionCovenant(fd);
		if result then
			return result;
		end
	end
	...
	then
		local result = Warlock:AfflictionSe(fd);
		if result then
			return result;
		end
	end
...
function Warlock:AfflictionAoe(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local currentSpell = fd.currentSpell;
...
function Warlock:AfflictionCovenant(fd)
	local cooldown = fd.cooldown;
	local debuff = fd.debuff;
	local currentSpell = fd.currentSpell;
	local talents = fd.talents;
...
function Warlock:AfflictionDarkglarePrep(fd)
	local cooldown = fd.cooldown;
	local currentSpell = fd.currentSpell;
	local talents = fd.talents;
	local soulShards = fd.soulShards;
...
function Warlock:AfflictionSe(fd)
	local cooldown = fd.cooldown;
	local currentSpell = fd.currentSpell;
	local talents = fd.talents;
	local debuff = fd.debuff;
