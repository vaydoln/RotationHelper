...
function Warlock:createDestructionEffectsTable()
   ...
   effects[DS.Incinerate] = function(fd)
      fd.soulShards = min(fd.soulShardsMax, fd.soulShards + 0.2);
      fd = RotationHelper:removeSelfBuff(fd, DS.Backdraft);
      return fd;
   end

   effects[DS.Conflagrate] = function(fd)
      fd.soulShards = min(fd.soulShardsMax, fd.soulShards + 0.5);
      fd = RotationHelper:addSelfBuff(fd, DS.Backdraft);
      fd = RotationHelper:startCooldown(fd, DS.Conflagrate);
      return fd;
   end

   effects[DS.Immolate] = function(fd)
      fd = RotationHelper:addTargetDebuff(fd, DS.ImmolateAura);
      return fd;
   end

   effects[DS.Cataclysm] = function(fd)
      fd = RotationHelper:addTargetDebuff(fd, DS.ImmolateAura);
      fd = RotationHelper:startCooldown(fd, DS.Cataclysm);
      return fd;
   end

   effects[DS.ChaosBolt] = function(fd)
      fd.soulShards = fd.soulShards - 2;
      fd = RotationHelper:removeSelfBuff(fd, DS.Backdraft);
      if (fd.talents[DS.Eradication]) then
         fd = RotationHelper:addSelfBuff(fd, DS.EradicationAura);
      end
      return fd;
   end

   effects[DS.SoulFire] = function(fd)
      fd.soulShards = min(fd.soulShardsMax, fd.soulShards + 1);
      fd = RotationHelper:startCooldown(fd, DS.SoulFire);
      return fd;
   end

   effects[DS.Shadowburn] = function(fd)
      fd.soulShards = fd.soulShards - 1;
      fd = RotationHelper:startCooldown(fd, DS.Shadowburn);
      return fd;
   end

   effects[DS.RainOfFire] = function(fd)
      fd.soulShards = fd.soulShards - 3;
      return fd;
   end
   ...
   effects[DS.ChannelDemonfire] = RotationHelper:normalCooldownEffect(DS.ChannelDemonfire);
   effects[DS.Havoc] = RotationHelper:normalCooldownEffect(DS.Havoc);
   effects[DS.SoulRot] = RotationHelper:normalCooldownEffect(DS.SoulRot);
   effects[DS.SummonInfernal] = RotationHelper:normalCooldownEffect(DS.SummonInfernal);
   effects[DS.DarkSoulInstability] = RotationHelper:normalCooldownEffect(DS.DarkSoulInstability);

   return effects;
end

local DSEffect = Warlock:createDestructionEffectsTable();

function Warlock:DestructionPrep(fd)
	fd.soulShards = UnitPower('player', Enum.PowerType.SoulShards, true) / 10;
   fd.soulShardsMax = 5;

   if (fd.currentSpell and DSEffect[fd.currentSpell]) then
      local updateFrameData = DSEffect[fd.currentSpell];
...
function Warlock:Destruction(fd)
   local spellId = Warlock:chooseDestructionSpellId(fd);
   local retVal = {
      id = spellId,
   };
...
function Warlock:chooseDestructionSpellId(fd)
	...
	local targetHp = RotationHelper:TargetPercentHealth();
	...
	-- summon_infernal;
	RotationHelper:GlowCooldown(DS.SummonInfernal, cooldown[DS.SummonInfernal].ready);
	RotationHelper:GlowCooldown(DS.Havoc, cooldown[DS.Havoc].ready);

	-- dark_soul_instability;
	if talents[DS.DarkSoulInstability] then
		RotationHelper:GlowCooldown(DS.DarkSoulInstability, cooldown[DS.DarkSoulInstability].ready);
	end
	...
	then
		result = Warlock:DestructionHavoc(fd);
		if result then
			return result;
		end
	end
	...
	if targets > 2 then
		result = Warlock:DestructionAoe(fd);
		if result then
			return result;
		end
	end
...
function Warlock:DestructionAoe(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local currentSpell = fd.currentSpell;
...
function Warlock:DestructionHavoc(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local currentSpell = fd.currentSpell;
