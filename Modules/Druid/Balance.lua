...
function Druid:createBalanceEffectsTable()
   ...
   effects[BL.Wrath] = function(fd)
      if (fd.eclipseInSolar) then
         if (fd.talents[BL.SoulOfTheForest]) then
            fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 9);
         else
            fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 6);
         end
      else
         fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 6);
         fd.wrathCount = fd.wrathCount - 1;
         if (fd.wrathCount <= 0) then
            fd = RotationHelper:addSelfBuff(fd, BL.EclipseLunar);
            fd.eclipseInLunar = true;
            fd.eclipseInAny = true;
            fd.eclipseInBoth = fd.eclipseInSolar;
         end
      end
      return fd;
   end

   effects[BL.Starfire] = function(fd)
      fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 8);
      if (not fd.eclipseInLunar) then
         fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 6);
         fd.starfireCount = fd.starfireCount - 1;
         if (fd.starfireCount <= 0) then
            fd = RotationHelper:addSelfBuff(fd, BL.EclipseSolar);
            fd.eclipseInSolar = true;
            fd.eclipseInAny = true;
            fd.eclipseInBoth = fd.eclipseInLunar;
         end
      end
      return fd;
   end

   effects[BL.Starsurge] = function(fd)
      fd.lunarPower = fd.lunarPower - 40;
      fd = RotationHelper:removeSelfBuff(fd, BL.OnethsPerception);
      return fd;
   end

   effects[BL.Starfall] = function(fd)
      fd.lunarPower = fd.lunarPower - 50;
      fd = RotationHelper:removeSelfBuff(fd, BL.OnethsClearVision);
      return fd;
   end

   effects[BL.Sunfire] = function(fd)
      fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 2);
      fd = RotationHelper:addTargetDebuff(fd, BL.SunfireAura);
      return fd;
   end

   effects[BL.Moonfire] = function(fd)
      fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 2);
      fd = RotationHelper:addTargetDebuff(fd, BL.MoonfireAura);
      return fd;
   end

   effects[BL.ForceOfNature] = function(fd)
      fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 20);
      fd = RotationHelper:startCooldown(fd, BL.ForceOfNature);
      return fd;
   end

   effects[BL.StellarFlare] = function(fd)
      fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 8);
      fd = RotationHelper:startCooldown(fd, BL.StellarFlare);
      return fd;
   end

   effects[BL.NewMoon] = function(fd)
      fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 10);
      fd.moonPhase = BL.HalfMoon;
      fd.castingMoonSpell = true;
      fd = RotationHelper:startCooldown(fd, BL.NewMoon);
      return fd;
   end

   effects[BL.HalfMoon] = function(fd)
      fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 20);
      fd.moonPhase = BL.FullMoon;
      fd.castingMoonSpell = true;
      fd = RotationHelper:startCooldown(fd, BL.NewMoon);
      return fd;
   end

   effects[BL.FullMoon] = function(fd)
      fd.lunarPower = min(fd.lunarPowerMax, fd.lunarPower + 40);
      fd.moonPhase = BL.NewMoon;
      fd.castingMoonSpell = true;
      fd = RotationHelper:startCooldown(fd, BL.NewMoon);
      return fd;
   end
   ...
   effects[BL.ConvokeTheSpirits] = RotationHelper:normalCooldownEffect(BL.ConvokeTheSpirits);
   effects[BL.KindredSpirits] = RotationHelper:normalCooldownEffect(BL.KindredSpirits);
   effects[BL.RavenousFrenzy] = RotationHelper:normalCooldownEffect(BL.RavenousFrenzy);

   return effects;
end

local DSEffect = Druid:createBalanceEffectsTable();

function Druid:BalancePrep(fd)
	fd.lunarPower = UnitPower('player', Enum.PowerType.LunarPower);
   fd.lunarPowerMax = 100;

   if (RotationHelper:FindSpell(BL.NewMoon)) then
      fd.moonPhase = BL.NewMoon;
   elseif (RotationHelper:FindSpell(BL.HalfMoon)) then
      fd.moonPhase = BL.HalfMoon;
   elseif (RotationHelper:FindSpell(BL.FullMoon)) then
      fd.moonPhase = BL.FullMoon;
   else
      fd.moonPhase = 0;
   end
	...
	fd.eclipseInLunar = buff[BL.EclipseLunar].up or (origWrathCount == 1 and currentSpell == BL.Wrath);
	fd.eclipseInSolar = buff[BL.EclipseSolar].up or (origStarfireCount == 1 and currentSpell == BL.Starfire);
	...
	RotationHelper:GlowCooldown(fd.CaInc, fd.cooldown[fd.CaInc].ready);
	if fd.talents[BL.ForceOfNature] then
		RotationHelper:GlowCooldown(BL.ForceOfNature, fd.cooldown[BL.ForceOfNature].ready);
	end

	if fd.covenant.covenantId == Venthyr then
		RotationHelper:GlowCooldown(BL.RavenousFrenzy, fd.cooldown[BL.RavenousFrenzy].ready);
	elseif fd.covenant.covenantId == NightFae then
		RotationHelper:GlowCooldown(BL.ConvokeTheSpirits, fd.cooldown[BL.ConvokeTheSpirits].ready);
	elseif fd.covenant.covenantId == Kyrian then
		RotationHelper:GlowCooldown(BL.KindredSpirits, fd.cooldown[BL.KindredSpirits].ready);
   end

   return fd;
end

function Druid:BalanceStep(fd)
   fd.castingMoonSpell = false;

   local solarLost = fd.removedAuras[BL.EclipseSolar];
   local lunarLost = fd.removedAuras[BL.EclipseLunar];
   if (solarLost and lunarLost) then
      fd.starfireCount = 2;
      fd.wrathCount = 2;
      fd.eclipseInLunar = false;
      fd.eclipseInSolar = false;
      fd.eclipseSolarNext = false;
      fd.eclipseLunarNext = false;
      fd.eclipseAnyNext = true;
   elseif (lunarLost) then
      fd.starfireCount = 2;
      fd.wrathCount = 0;
      fd.eclipseInSolar = false;
      fd.eclipseSolarNext = true;
      fd.eclipseLunarNext = false;
      fd.eclipseAnyNext = false;
   elseif (solarLost) then
      fd.starfireCount = 0;
      fd.wrathCount = 2;
      fd.eclipseInLunar = false;
      fd.eclipseSolarNext = true;
...
function Druid:Balance(fd)
   local spellId = Druid:chooseBalanceSpellId(fd);
   local retVal = {
      id = spellId,
   };
...
function Druid:chooseBalanceSpellId(fd)
	-- run_action_list,name=aoe,if=variable.is_aoe;
	if fd.isAoe then
		return Druid:BalanceAoe(fd);
	end

	-- run_action_list,name=dreambinder,if=runeforge.timeworn_dreambinder.equipped;
	if fd.runeforge[BL.TimewornDreambinderBonusId] then
		return Druid:BalanceDreambinder(fd);
	end

	-- run_action_list,name=boat,if=runeforge.balance_of_all_things.equipped;
	if fd.runeforge[BL.BalanceOfAllThingsBonusId] then
		return Druid:BalanceBoat(fd);
	end

	-- run_action_list,name=st;
	return Druid:BalanceSt(fd);
end

function Druid:BalanceAoe(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
...
function Druid:BalanceDreambinder(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local currentSpell = fd.currentSpell;
...
function Druid:BalanceFallthru(fd)
	local runeforge = fd.runeforge;
	local lunarPower = fd.lunarPower;
	local debuff = fd.debuff;
...
function Druid:BalanceSt(fd)
	...
	--
	---- run_action_list,name=fallthru;
	--return Druid:BalanceFallthru(fd);
end
