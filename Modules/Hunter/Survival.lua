...
function Hunter:SurvivalBombId()
	if RotationHelper:FindSpell(SV.PheromoneBomb) then
		return SV.PheromoneBomb;
	elseif RotationHelper:FindSpell(SV.VolatileBomb) then
		return SV.VolatileBomb;
	elseif RotationHelper:FindSpell(SV.ShrapnelBomb) then
...
function Hunter:createSurvivalEffectsTable()
   local effects = {};

   -- TODO: spec - survival hunter

   -- effects[SV.BarbedShot] = function(fd)
   ...
   -- effects[SV.Multishot] = function(fd)
   --    fd.focus = fd.focus - 40;
   --    return fd;
   -- end

   -- effects[SV.ResonatingArrow] = RotationHelper:normalCooldownEffect(SV.ResonatingArrow);

   return effects;
end

local SVEffect = Hunter:createSurvivalEffectsTable();

function Hunter:SurvivalAfterSpell(fd)
   return fd;
end

function Hunter:SurvivalPrep(fd)
	fd.focus, fd.focusMax, fd.focusRegen = Hunter:Focus(0, fd.timeShift);
	fd.targets = RotationHelper:SmartAoe();

   if (fd.currentSpell and SVEffect[fd.currentSpell]) then
      local updateFrameData = SVEffect[fd.currentSpell];
...
function Hunter:Survival(fd)
   local spellId = Hunter:chooseSurvivalSpellId(fd);
   local retVal = {
      id = spellId,
   };

   if (spellId and SVEffect[spellId]) then
      retVal.updateFrameData = SVEffect[spellId];
   end

   return retVal;
end

function Hunter:chooseSurvivalSpellId(fd)
	...
	if fd.targets < 3 and talents[SV.WildfireInfusion] and talents[SV.AlphaPredator] and talents[SV.MongooseBite] then
		return Hunter:SurvivalMbApWfiSt(fd);
	end

	if fd.targets < 3 and talents[SV.WildfireInfusion] then
		return Hunter:SurvivalWfiSt(fd);
	end

	if fd.targets < 2 then
		return Hunter:SurvivalSingleTarget(fd);
	end

	if fd.targets > 1 then
		return Hunter:SurvivalCleave(fd);
	end
end

function Hunter:Cds(fd)
	local cooldown = fd.cooldown;

	RotationHelper:GlowEssences(fd);

	RotationHelper:GlowCooldown(SV.CoordinatedAssault, cooldown[SV.CoordinatedAssault].ready);
...
function Hunter:SurvivalCleave(fd)
	...
	local MongooseBite = RotationHelper:FindSpell(265888) and 265888 or SV.MongooseBite;
	local RaptorStrike = RotationHelper:FindSpell(265189) and 265189 or SV.RaptorStrike;
...
function Hunter:SurvivalMbApWfiSt(fd)
	local cooldown, buff, debuff, talents, azerite, currentSpell, targets, focus, focusMax, focusRegen, gcd, timeShift =
		fd.cooldown, fd.buff, fd.debuff, fd.talents, fd.azerite, fd.currentSpell, fd.targets, fd.focus, fd.focusMax, fd.focusRegen, fd.gcd, fd.timeShift;
...
function Hunter:SurvivalSingleTarget(fd)
	local cooldown, buff, debuff, talents, azerite, currentSpell, targets, focus, focusMax, focusRegen, gcd, timeShift =
	fd.cooldown, fd.buff, fd.debuff, fd.talents, fd.azerite, fd.currentSpell, fd.targets, fd.focus, fd.focusMax, fd.focusRegen, fd.gcd, fd.timeShift;
...
function Hunter:SurvivalWfiSt(fd)
	local cooldown, buff, debuff, talents, azerite, currentSpell, targets, focus, focusMax, focusRegen, gcd, timeShift =
	fd.cooldown, fd.buff, fd.debuff, fd.talents, fd.azerite, fd.currentSpell, fd.targets, fd.focus, fd.focusMax, fd.focusRegen, fd.gcd, fd.timeShift;
