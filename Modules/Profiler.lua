...
local Profiler = RotationHelper:NewModule('Profiler', 'AceEvent-3.0');
...
function Profiler:StartProfiler()
	...
	RotationHelper:Print(RotationHelper.Colors.Info .. 'Profiler started');
end
...
function Profiler:StopProfiler()
	...
	self:ShowWindow();
	RotationHelper:Print(RotationHelper.Colors.Success .. 'Profiler finished');
end
...
function Profiler:ShowWindow()
	...
	local f = StdUi:Window(UIParent, 500, 600, 'RotationHelper Profiler');
	f:SetPoint('CENTER');
