...
function Mage:createFrostEffectsTable()
   local effects = {};

   -- TODO: spec - frost mage

   -- effects[FT.BarbedShot] = function(fd)
   --    fd = RotationHelper:startCooldown(fd, FT.BarbedShot);
   --    fd = RotationHelper:reduceCooldown(fd, FT.BestialWrath, 12);
   --    fd = RotationHelper:addTargetDebuff(fd, FT.BarbedShotAura);
   --    fd.pet = RotationHelper:addAura(fd.pet, FT.Frenzy);
   --    return fd;
   -- end

   -- effects[FT.Multishot] = function(fd)
   --    fd.focus = fd.focus - 40;
   --    return fd;
   -- end

   -- effects[FT.ResonatingArrow] = RotationHelper:normalCooldownEffect(FT.ResonatingArrow);

   return effects;
end

local FTEffect = Mage:createFrostEffectsTable();

function Mage:FrostAfterSpell(fd)
   return fd;
end

function Mage:FrostPrep(fd)
	fd.targets = RotationHelper:SmartAoe();

	local currentSpell  = fd.currentSpell;
	local icicles = fd.buff[FT.Icicles].count;
   ...
   fd.icicles = icicles;
   fd.hasPet = UnitExists('pet');

   if (fd.currentSpell and FTEffect[fd.currentSpell]) then
      local updateFrameData = FTEffect[fd.currentSpell];
      fd = updateFrameData(fd);
   end

	RotationHelper:GlowEssences(fd);
	RotationHelper:GlowCooldown(FT.MirrorImage, fd.talents[FT.MirrorImage] and fd.cooldown[FT.MirrorImage].ready);
	RotationHelper:GlowCooldown(FT.IcyVeins, fd.cooldown[FT.IcyVeins].ready);

   return fd;
end

function Mage:Frost(fd)
   local spellId = Mage:chooseFrostSpellId(fd);
   local retVal = {
      id = spellId,
   };

   if (spellId and FTEffect[spellId]) then
      retVal.updateFrameData = FTEffect[spellId];
   end

   return retVal;
end

function Mage:chooseFrostSpellId(fd)
	local buff = fd.buff;
	...
	local currentSpell  = fd.currentSpell;
	local canRop = talents[FT.RuneOfPower] and currentSpell ~= FT.RuneOfPower and Mage:CanRuneOfPower(FT.RuneOfPower, fd.timeShift) <= 0;

	local FrozenOrb = RotationHelper:FindSpell(FT.FrozenOrb2) and FT.FrozenOrb2 or FT.FrozenOrb;

	fd.FrozenOrb = FrozenOrb;
	fd.targets = targets;
	fd.canRop = canRop;

	if not talents[FT.LonelyWinter] and not fd.hasPet
	...
	local result = Mage:FrostCooldowns(fd);
	...
	if targets > 3 and talents[FT.FreezingRain] or targets > 4 then
		result = Mage:FrostAoe(fd);
		if result then return result; end
	end

	-- call_action_list,name=single;
	return Mage:FrostSingle(fd);
end

function Mage:FrostAoe(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local currentSpell = fd.currentSpell;
...
function Mage:FrostCooldowns(fd)
	local cooldown = fd.cooldown;
	local spellHistory = fd.spellHistory;
	local talents = fd.talents;
	local targets = fd.targets;
...
function Mage:FrostTalentRop(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local talents = fd.talents;
	local icicles = fd.icicles;
