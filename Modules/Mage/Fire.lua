...
function Mage:createFireEffectsTable()
   local effects = {};

   -- TODO: spec - fire mage

   -- effects[FR.BarbedShot] = function(fd)
   ...
   -- effects[FR.Multishot] = function(fd)
   --    fd.focus = fd.focus - 40;
   --    return fd;
   -- end

   -- effects[FR.ResonatingArrow] = RotationHelper:normalCooldownEffect(FR.ResonatingArrow);

   return effects;
end

local FREffect = Mage:createFireEffectsTable();

function Mage:FireAfterSpell(fd)
   return fd;
end

function Mage:FirePrep(fd)
	fd.targets = RotationHelper:SmartAoe();
   fd.combustionRopCutoff = 60;
   fd.targetHp = RotationHelper:TargetPercentHealth() * 100;
   fd.firestarterActive = fd.talents[FR.Firestarter] and fd.targetHp > 90
...
function Mage:Fire(fd)
   local spellId = Mage:chooseFireSpellId(fd);
   local retVal = {
      id = spellId,
   };
...
function Mage:GlowFireCds(fd)
  ...
  RotationHelper:GlowCooldown(FR.MirrorImage, cooldown[FR.MirrorImage].ready and not buff[FR.Combustion].up);
	...
	if talents[FR.RuneOfPower] then
		RotationHelper:GlowCooldown(FR.RuneOfPower,
	...
	RotationHelper:GlowCooldown(FR.Combustion,
...
function Mage:chooseFireSpellId(fd)
	local buff = fd.buff;
	...
	if buff[FR.Combustion].up then
		local result = Mage:FireCombustionPhase(fd);
	...
	if buff[FR.RuneOfPowerAura].up and not buff[FR.Combustion].up then
		local result = Mage:FireRopPhase(fd);
		if result then return result; end
	end

	-- call_action_list,name=standard_rotation;
	return Mage:FireStandardRotation(fd);
end

function Mage:FireActiveTalents(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local talents = fd.talents;
...
function Mage:FireBmCombustionPhase(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local currentSpell = fd.currentSpell;
	local talents = fd.talents;
...
function Mage:FireCombustionPhase(fd)
	local cooldown = fd.cooldown;
...
function Mage:FireStandardRotation(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local currentSpell = fd.currentSpell;
	local talents = fd.talents;
