...
function DeathKnight:createUnholyEffectsTable()
   ...
   effects[UH.UnholyBlight] = function(fd)
      fd = DeathKnight:spendRunes(fd, 1);
      fd = RotationHelper:startCooldown(fd, UH.UnholyBlight);
      fd = RotationHelper:addTargetDebuff(fd, UH.VirulentPlague);
      fd = RotationHelper:addTargetDebuff(fd, UH.UnholyBlightDot);
      return fd;
   end

   effects[UH.SoulReaper] = function(fd)
      fd = DeathKnight:spendRunes(fd, 1);
      fd = RotationHelper:startCooldown(fd, UH.SoulReaper);
      fd = RotationHelper:addTargetDebuff(fd, UH.SoulReaper);
      return fd;
   end

   effects[UH.Defile] = function(fd)
      fd = DeathKnight:spendRunes(fd, 1);
      fd = RotationHelper:startCooldown(fd, UH.Defile);
      return fd;
   end

   effects[UH.DeathAndDecay] = function(fd)
      fd = DeathKnight:spendRunes(fd, 1);
      fd = RotationHelper:startCooldown(fd, UH.DeathAndDecay);
      fd = RotationHelper:addSelfBuff(fd, UH.DeathAndDecayBuff);
      return fd;
   end

   effects[UH.Outbreak] = function(fd)
      fd = DeathKnight:spendRunes(fd, 1);
      fd = RotationHelper:addTargetDebuff(fd, UH.VirulentPlague);
      return fd;
   end

   effects[UH.FesteringStrike] = function(fd)
      fd = DeathKnight:spendRunes(fd, 2);
      fd = DeathKnight:addFesteringWound(fd, 2);
      return fd;
   end

   effects[UH.UnholyAssault] = function(fd)
      fd = RotationHelper:startCooldown(fd, UH.UnholyAssault);
      fd = RotationHelper:addSelfBuff(fd, UH.UnholyAssault);
      fd = DeathKnight:addFesteringWound(fd, 4);

      return fd;
   end

   effects[UH.ScourgeStrike] = function(fd)
      fd = DeathKnight:spendRunes(fd, 1);
      fd = DeathKnight:removeFesteringWound(fd, 1);
      return fd;
   end

   effects[UH.ClawingShadows] = function(fd)
      fd = DeathKnight:spendRunes(fd, 1);
      fd = DeathKnight:removeFesteringWound(fd, 1);
      return fd;
   end

   effects[UH.Epidemic] = function(fd)
      fd.runicPower = fd.runicPower - 30;
      return fd;
   end

   effects[UH.DeathCoil] = function(fd)
      fd.runicPower = fd.runicPower - 40;
      fd = RotationHelper:reduceCooldown(fd, UH.DarkTransformation, 1);

      if (fd.talents[UH.ArmyOfTheDamned]) then
         fd = RotationHelper:reduceCooldown(fd, UH.Apocalypse, 1);
         fd = RotationHelper:reduceCooldown(fd, UH.ArmyOfTheDead, 5);
      end

      return fd;
   end

   effects[UH.Apocalypse] = function(fd)
      fd = RotationHelper:startCooldown(fd, UH.Apocalypse);
      fd = DeathKnight:removeFesteringWound(fd, 4);
      return fd;
   end

   effects[UH.DarkTransformation] = function(fd)
      fd = RotationHelper:startCooldown(fd, UH.DarkTransformation);
      fd.pet = RotationHelper:addAura(fd.pet, UH.DarkTransformation);
      return fd;
   end

   effects[UH.SacrificialPact] = function(fd)
      fd.runicPower = fd.runicPower - 20;
      fd = RotationHelper:startCooldown(fd, UH.SacrificialPact);
      fd.petExists = false;
      return fd;
   end

   effects[UH.RaiseDead] = function(fd)
      fd = RotationHelper:startCooldown(fd, UH.RaiseDead);
      fd.petExists = true;
      return fd;
   end

   effects[UH.AbominationLimb] = RotationHelper:normalCooldownEffect(UH.AbominationLimb);
   effects[UH.SummonGargoyle] = RotationHelper:normalCooldownEffect(UH.SummonGargoyle);

   return effects;
end

local UHEffect = DeathKnight:createUnholyEffectsTable();

function DeathKnight:addFesteringWound(fd, count)
   local maxCount = 6;
   if (fd.debuff[UH.FesteringWound].up) then
      maxCount = 6 - fd.debuff[UH.FesteringWound].count;
   end
...
function DeathKnight:removeFesteringWound(fd, count)
   if (fd.debuff[UH.FesteringWound].up) then
...
function DeathKnight:UnholyPrep(fd)
   fd = DeathKnight:prepRunes(fd);
   fd.targets = RotationHelper:SmartAoe();
	fd.petExists = UnitExists('pet');
	fd.deathKnightFwoundedTargets = DeathKnight:WoundedTargets(fd);
   fd.deathKnightDisableAotd = DeathKnight.db.unholyArmyOfTheDeadAsCooldown;

   fd.pet = {};
   fd.pet[UH.ControlUndead] = RotationHelper:IntUnitAura('pet', UH.ControlUndead, 'PLAYER', fd.timeShift);
   fd.pet[UH.DarkTransformation] = RotationHelper:IntUnitAura('pet', UH.DarkTransformation, nil, fd.timeShift);

   if (fd.currentSpell and UHEffect[fd.currentSpell]) then
      local updateFrameData = UHEffect[fd.currentSpell];
...
function DeathKnight:Unholy(fd)
   local spellId = DeathKnight:chooseUnholySpellId(fd);
   local retVal = {
      id = spellId,
   };
...
function DeathKnight:chooseUnholySpellId(fd)
	local gcd = fd.gcd;
   ...
   if (fd.recurseIndex == 1) then
      DeathKnight:UholyGlowCooldowns(fd);
   end
	...
	local result = DeathKnight:UnholyCovenants(fd);
	if result then
		return result;
	end

	-- call_action_list,name=cooldowns;
	result = DeathKnight:UnholyCooldowns(fd);
	if result then
		return result;
	end
	...
	then
		return DeathKnight:UnholyAoeSetup(fd);
	end
	...
	if targets >= 2 and buff[UH.DeathAndDecayBuff].up then
		return DeathKnight:UnholyAoeBurst(fd);
	end
	...
	then
		return DeathKnight:UnholyGenericAoe(fd);
	end

	-- call_action_list,name=generic,if=active_enemies=1;
	if targets <= 1 then
		result = DeathKnight:UnholyGeneric(fd);
		if result then
			return result;
		end
	end
...
function DeathKnight:UholyGlowCooldowns(fd)
	local cooldown = fd.cooldown;
	local debuff = fd.debuff;
	local talents = fd.talents;
	local buff = fd.buff;
	...
	if DeathKnight.db.alwaysGlowCooldowns then
		RotationHelper:GlowCooldown(UH.ArmyOfTheDead, armyOfTheDeadReady);
		RotationHelper:GlowCooldown(UH.SummonGargoyle, summonGargoyleReady);
		RotationHelper:GlowCooldown(UH.AbominationLimb, abominationLimbReady);
	else
		...
		RotationHelper:GlowCooldown(UH.ArmyOfTheDead, armyOfTheDeadCooldownTrigger);
		RotationHelper:GlowCooldown(UH.SummonGargoyle, summonGargoyleCooldownTrigger);
		RotationHelper:GlowCooldown(UH.AbominationLimb, abominationLimbCooldownTrigger);
	end
end

function DeathKnight:UnholyAoeBurst(fd)
	local buff = fd.buff;
	local talents = fd.talents;
	local targets = fd.targets;
	local runes = fd.runes;
	...
	local covenantId = fd.covenant.covenantId
	local darkTransformationAura = fd.pet[UH.DarkTransformation];
...
function DeathKnight:UnholyAoeSetup(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local targets = fd.targets;
	...
	--[[
		NOTE: The SIMC priority list includes selectively applying Festering Wounds to multiple targets, and casting Death and Decay when 5+ targets are wounded,
			  ...
			  Since RotationHelper can't do target-switching for us, we'll approximate this behavior by casting Death and Decay when the current target has 5+ wounds.
	  ]]
...
function DeathKnight:UnholyCooldowns(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local talents = fd.talents;
	...
	local controlUndeadAura = fd.pet[UH.ControlUndead];
	local darkTransformationAura = fd.pet[UH.DarkTransformation];
	local targetHpPercent = RotationHelper:TargetPercentHealth() * 100;
...
function DeathKnight:UnholyCovenants(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local talents = fd.talents;
...
function DeathKnight:UnholyGeneric(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local talents = fd.talents;
...
function DeathKnight:UnholyGenericAoe(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local targets = fd.targets;
...
function DeathKnight:WoundedTargets(fd)
	local _, units = RotationHelper:ThreatCounter();
	local count = 0;

	for i = 1, #units do
		local festeringWound = RotationHelper:IntUnitAura(tostring(units[i]), UH.FesteringWound, 'PLAYER', fd.timeShift)
