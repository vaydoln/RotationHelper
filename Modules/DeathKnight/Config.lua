			...
			[12] = {
				infoText = {
					...
					label = 'LEAVE THIS CHECKED UNLESS YOU\'VE READ AND UNDERSTOOD THIS DISCLAIMER! ' ..
						...
						'you\'ve manually met the requirements, and RotationHelper might not suggest actions ' ..
						'that build toward those requirements. If you don\'t want to ignore RotationHelper ' ..
...
function DeathKnight:InitializeDatabase()
	...
	if not RotationHelperDeathKnightOptions then
		RotationHelperDeathKnightOptions = defaultOptions;
	end

	for k, v in pairs(defaultOptions) do
		if RotationHelperDeathKnightOptions[k] == nil then
			RotationHelperDeathKnightOptions[k] = v;
		end
	end

	self.db = RotationHelperDeathKnightOptions;
end
...
function DeathKnight:CreateConfig()
	...
	optionsFrame.parent = 'RotationHelper';
