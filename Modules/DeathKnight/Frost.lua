...
function DeathKnight:createFrostEffectsTable()
   local effects = {};

   effects[FR.HowlingBlast] = function(fd)
      fd = DeathKnight:spendRunesFrost(fd, 1);
      fd = RotationHelper:addTargetDebuff(fd, FR.HowlingBlast);
      fd = RotationHelper:removeSelfBuff(fd, FR.Rime);
      return fd;
   end

   effects[FR.Obliterate] = function(fd)
      fd = DeathKnight:spendRunesFrost(fd, 2);
      fd = RotationHelper:addTargetDebuff(fd, FR.FrostFever);
      return fd;
   end

   effects[FR.FrostStrike] = function(fd)
      fd = DeathKnight:spendRunicPowerFrost(fd, 25);
      return fd;
   end

   effects[FR.HornOfWinter] = function(fd)
      fd = RotationHelper:startCooldown(fd, FR.HornOfWinter);
      fd.runicPower = min(fd.runicPowerMax, fd.runicPower + 25);
      fd = DeathKnight:gainRunes(fd, 2);
      return fd;
   end

   effects[FR.SacrificialPact] = function(fd)
      fd = DeathKnight:spendRunicPowerFrost(fd, 20);
      fd = RotationHelper:startCooldown(fd, FR.SacrificialPact);
      fd.petExists = false;
      return fd;
   end

   effects[FR.RaiseDead] = function(fd)
      fd = RotationHelper:startCooldown(fd, FR.RaiseDead);
      fd.petExists = true;
      return fd;
   end

   effects[FR.ChainsOfIce] = function(fd)
      fd = DeathKnight:spendRunesFrost(fd, 1);
      fd = RotationHelper:removeSelfBuff(fd, FR.ColdHeartBuff, 20);
      return fd;
   end

   effects[FR.Frostscythe] = function(fd)
      fd = DeathKnight:spendRunesFrost(fd, 1);
      return fd;
   end

   effects[FR.DeathAndDecay] = function(fd)
      fd = RotationHelper:startCooldown(fd, FR.DeathAndDecay);
      fd = RotationHelper:addSelfBuff(fd, FR.DeathAndDecayBuff);
      fd = DeathKnight:spendRunesFrost(fd, 1);
      return fd;
   end

   effects[FR.BreathOfSindragosa] = function(fd)
      fd = RotationHelper:startCooldown(fd, FR.BreathOfSindragosa);
      fd = RotationHelper:addSelfBuff(fd, FR.BreathOfSindragosa);
      fd = DeathKnight:gainRunes(fd, 2);
      return fd;
   end

   effects[FR.HypothermicPresence] = function(fd)
      fd = RotationHelper:startCooldown(fd, FR.HypothermicPresence);
      fd = RotationHelper:addSelfBuff(fd, FR.HypothermicPresence);
      return fd;
   end

   effects[FR.PillarOfFrost] = function(fd)
      fd = RotationHelper:startCooldown(fd, FR.PillarOfFrost);
      fd = RotationHelper:addSelfBuff(fd, FR.PillarOfFrost);
      return fd;
   end

   effects[FR.GlacialAdvance] = function(fd)
      fd = RotationHelper:startCooldown(fd, FR.GlacialAdvance);
      fd = DeathKnight:spendRunicPowerFrost(fd, 30);
      return fd;
   end

   effects[FR.EmpowerRuneWeapon] = function(fd)
      fd = RotationHelper:startCooldown(fd, FR.EmpowerRuneWeapon);
      fd = DeathKnight:gainRunes(fd, 1);
      fd.runicPower = min(fd.runicPowerMax, fd.runicPower + 5);
      return fd;
   end

   effects[FR.AbominationLimb] = RotationHelper:normalCooldownEffect(FR.AbominationLimb);
   effects[FR.ShackleTheUnworthy] = RotationHelper:normalCooldownEffect(FR.ShackleTheUnworthy);
   effects[FR.SwarmingMist] = RotationHelper:normalCooldownEffect(FR.SwarmingMist);

   return effects;
end

function DeathKnight:spendRunicPowerFrost(fd, cost)
   if (fd.buff[FR.HypothermicPresence].up) then
      cost = cost * 0.65;
   end
   fd.runicPower = fd.runicPower - cost;
   return fd;
end

function DeathKnight:spendRunesFrost(fd, runeCount)
   fd = DeathKnight:spendRunes(fd, runeCount);

   if (fd.talents[FR.GatheringStorm]) then
      local remorselessWinterBuff = fd.buff[FR.RemorselessWinter];
      if (remorselessWinterBuff.up) then
         remorselessWinterBuff.remains = remorselessWinterBuff.remains + (runeCount * 0.5);
      end
   end
...
local FREffect = DeathKnight:createFrostEffectsTable();

function DeathKnight:FrostAfterSpell(fd)
   if (fd.talents[FR.ColdHeart]) then
      -- this will just add 1 stack, but with GCD and other
      -- considerations it might not be just 1 (oh well)
      fd = RotationHelper:addSelfBuff(fd, FR.ColdHeartBuff);
   end

   if (fd.removedAuras[FR.BreathOfSindragosa]) then
      fd = DeathKnight:gainRunes(fd, 2);
   elseif (fd.buff[FR.BreathOfSindragosa].up) then
      fd = DeathKnight:spendRunicPowerFrost(fd, 16);
   end

   return fd;
end

function DeathKnight:FrostPrep(fd)
   ...
   if (fd.currentSpell and FREffect[fd.currentSpell]) then
	...
	DeathKnight:FrostGlowCooldowns(fd);

   return fd;
end

function DeathKnight:Frost(fd)
   local spellId = DeathKnight:chooseFrostSpellId(fd);
   local retVal = {
      id = spellId,
   };
...
function DeathKnight:chooseFrostSpellId(fd)
	...
	result = DeathKnight:FrostCovenants(fd);
	if result then
		return result;
	end

	-- call_action_list,name=cooldowns;
	result = DeathKnight:FrostCooldowns(fd);
	if result then
		return result;
	end
	...
	-- or fightRemains <= gcd )
	then
		result = DeathKnight:FrostColdHeart(fd);
		if result then
			return result;
		end
	end
	...
	if buff[FR.BreathOfSindragosa].up then
		return DeathKnight:FrostBosTicking(fd);
	end
	...
	if talents[FR.BreathOfSindragosa] and cooldown[FR.BreathOfSindragosa].remains < 10 then
		return DeathKnight:FrostBosPooling(fd);
	end
	...
	if buff[FR.PillarOfFrost].up and talents[FR.Obliteration] then
		return DeathKnight:FrostObliteration(fd);
	end
	...
	if talents[FR.Obliteration] and cooldown[FR.PillarOfFrost].remains < 10 then
		return DeathKnight:FrostObliterationPooling(fd);
	end

	-- run_action_list,name=aoe,if=active_enemies>=2;
	if targets >= 2 then
		return DeathKnight:FrostAoe(fd);
	end

	-- call_action_list,name=standard;
	result = DeathKnight:FrostStandard(fd);
	if result then
		return result;
	end
end

function DeathKnight:FrostGlowCooldowns(fd)
	local gcd = fd.gcd;
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	...
	if DeathKnight.db.alwaysGlowCooldowns then
		RotationHelper:GlowCooldown(FR.FrostwyrmsFury, frostwyrmsFuryReady);
		RotationHelper:GlowCooldown(FR.AbominationLimb, abominationLimbReady);
	else
		...
		RotationHelper:GlowCooldown(FR.FrostwyrmsFury, frostwyrmsFuryCooldownTrigger);
		RotationHelper:GlowCooldown(FR.AbominationLimb, abominationLimbCooldownTrigger);
	end
end

function DeathKnight:FrostAoe(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local talents = fd.talents;
	local gcd = fd.gcd;
...
function DeathKnight:FrostBosPooling(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local talents = fd.talents;
	local targets = fd.targets;
...
function DeathKnight:FrostBosTicking(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local talents = fd.talents;
	local targets = fd.targets;
...
function DeathKnight:FrostColdHeart(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local talents = fd.talents;
	local runes = fd.runes;
...
function DeathKnight:FrostCooldowns(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local talents = fd.talents;
...
function DeathKnight:FrostCovenants(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local talents = fd.talents;
	local targets = fd.targets;
...
function DeathKnight:FrostObliteration(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local talents = fd.talents;
...
function DeathKnight:FrostObliterationPooling(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local talents = fd.talents;
	local targets = fd.targets;
...
function DeathKnight:FrostStandard(fd)
	local cooldown = fd.cooldown;
	local buff = fd.buff;
	local debuff = fd.debuff;
	local talents = fd.talents;
